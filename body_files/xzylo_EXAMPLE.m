function [aero,initialization] = xzylo_EXAMPLE()
    ...
    aero.CD = @(alpha,beta) 0.005 + 0.5*alpha^2;
    aero.CL = @(alpha,beta) 1.2*alpha;
    aero.CY = @(alpha,beta) -1.2*beta;
    aero.Cl = @(alpha,beta) -0*alpha;
    aero.Cm = @(alpha,beta) -0.001 + 0*alpha;
    aero.Cn = @(alpha,beta) -0.001 + 0*beta;
    ...
    % --- Initial State ---
    initialization.v0 = [17 0 0]; % initial velocity vector (u,v,w)_0
    initialization.omega0 = [-25 0 0]; % initial rotational velocity vector (p,q,r)_0
    initialization.quat0 = [1 0 0 0];  % unit quaternion
    initialization.pos0 = [0 0 100]; % initial position in inertial frame
